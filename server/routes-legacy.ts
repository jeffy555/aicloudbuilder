...
5. If user requests resources, your response should be action-oriented and brief, not educational.`;
  ...
  // NOTE: /api/repositories/:provider routes (GET, POST, prewarm) have been
  // fully migrated to server/routes/repositories.ts which handles Bitwarden
  // credential lookup. These duplicate routes have been removed to prevent
  // the env-var-only check from shadowing the Bitwarden-aware modular route.
