...
/**
 * Smart fallback cost calculation for resources without a pricing config.
 * Interprets the unitOfMeasure to calculate a reasonable monthly cost.
 */
export function calculateFallbackMonthlyCost(item: any): number {
  if (!item || !item.retailPrice) return 0;

  const unit = (item.unitOfMeasure || '').toLowerCase();
  const price = item.retailPrice;

  if (unit.includes('hour') || unit === '1 hour') {
    return price * HOURS_PER_MONTH;
  }
  if (unit.includes('day') || unit === '1 day') {
    return price * 30;
  }
  if (unit.includes('month') || unit === '1/month') {
    return price;
  }
  ...
  if (unit.includes('gb')) {
    return price * 50; // Estimate 50GB usage
  }
  if (unit.includes('10k') || unit.includes('10,000')) {
    return price * 1; // 10,000 operations
  }
  if (unit.includes('1m') || unit.includes('million') || unit.includes('1,000,000')) {
    return price * 1; // 1M operations
  }
  if (unit.includes('100')) {
    return price * 100; // 10,000 operations
  }
  // Default: treat as hourly if price is small, monthly if larger
  if (price < 1) {
    return price * HOURS_PER_MONTH;
  }
  return price;
